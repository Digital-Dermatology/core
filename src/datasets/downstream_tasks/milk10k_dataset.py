...
def _modality_from_image_type(image_type) -> str:
    """Map an ISIC-archive ``image_type`` string to a SkinMap modality.

    MILK10k pairs a clinical close-up and a dermatoscopic image for every
    lesion, so the modality has to be read per image rather than per dataset.
    """
...
class MILK10kDataset(GenericImageDataset):
    """MILK10k (multimodal imaging-learning toolkit) image dataset.

    A 5-center, malignant-rich dermatology dataset of 10,480 images (5,240
    paired clinical close-up + dermatoscopic lesions, 95.7% biopsied) hosted on
    the ISIC archive. Expects the ISIC-archive metadata export (same schema as
    MSKCC/HIBA) with images under ``<dataset_dir>/images``.
    """
    ...
    def __init__(
        ...
        milk10k_meta_name: Union[str, Path] = "milk10k.csv",
        transform=None,
        val_transform=None,
        ...
        **kwargs,
        ...
        """
        Initializes the MILK10k dataset.

        Parameters
        ----------
        dataset_dir : str
            Directory with all the images.
        milk10k_meta_name : str
        ...
        self.meta_data = pd.read_csv(self.dataset_dir / milk10k_meta_name)
        ...
        def create_description(row):
            ...
            # Image type (varies per row: dermatoscopic / clinical close-up)
            image_type = (
                row.get("image_type")
        ...
        # Per-image modality (dermatoscopic vs clinical close-up) is read from
        # the ISIC image_type field; the prep pipeline honors this column when
        # present and falls back to the dataset-level modality mapper otherwise.
        ...
        self.meta_data["dataset_desc"] = "MILK10k"

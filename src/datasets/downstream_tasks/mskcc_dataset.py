...
class MSKCCDataset(GenericImageDataset):
    """MSKCC (Memorial Sloan Kettering Cancer Center) image dataset."""
    ...
    def __init__(
        ...
        mskcc_meta_name: Union[str, Path] = "mskcc.csv",
        transform=None,
        val_transform=None,
        ...
        **kwargs,
        ...
        self.meta_data = pd.read_csv(self.dataset_dir / mskcc_meta_name)
        ...
        def create_description(row):
            ...
            # Melanoma-specific fields if present
            if pd.notna(row.get("mel_thick_mm")):
                parts.append(f"Melanoma thickness: {row.get('mel_thick_mm')} mm.")
            if pd.notna(row.get("mel_type")):
                parts.append(f"Melanoma type: {row.get('mel_type')}.")
        ...
        self.meta_data["dataset_desc"] = "MSKCC"

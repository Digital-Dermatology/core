...
class SCINDataset(BaseDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.meta_data["path"] = self.meta_data["image_1_path"].apply(
            lambda x: str(self.root.parent / x)
        )

        # Generate continuous-text descriptions for each row
        def _join_flags(row, prefix):
            items = [
                col.replace(prefix, "").lower().replace("_", " ")
                for col, val in row.items()
                if col.startswith(prefix) and val
            ]
        ...
        def _generate_description(row):
            ...
            fitz = (
                str(row.get("fitzpatrick_skin_type"))
                if pd.notna(row.get("fitzpatrick_skin_type"))
                else "unknown"
            )
            ...
            others = [
                col.replace("other_symptoms_", "").lower().replace("_", " ")
                for col, val in row.items()
                if col.startswith("other_symptoms_") and val
            ]
            other_txt = f"; also {', '.join(others)}" if others else ""

            fst_labels = [
                row[f"dermatologist_fitzpatrick_skin_type_label_{j}"]
                for j in (1, 2, 3)
            ...
            duration = (
                str(row.get("condition_duration"))
                if pd.notna(row.get("condition_duration"))
                else "unknown"
            )
            monk_india = (
                str(row.get("monk_skin_tone_label_india"))
                if pd.notna(row.get("monk_skin_tone_label_india"))
                else "n/a"
            )
            monk_us = (
                str(row.get("monk_skin_tone_label_us"))
                if pd.notna(row.get("monk_skin_tone_label_us"))
                else "n/a"
            )
    ...
    def __getitem__(self, idx):
        ...
        for i in range(1, 4):
            ...
            if isinstance(img_path, str) and os.path.isfile(
